...
// Function to detect user's country from browser locale
const detectUserCountry = (): string => {
  try {
    // Try to get country from Intl.Locale
    const locale = new Intl.Locale(navigator.language);
    if (locale.region) {
      return locale.region;
    }
    
    // Fallback to parsing navigator.language
...
const CountryItem = memo(({ 
  ...
  <Button
    ...
    variant="ghost"
    className={`w-full justify-start flex items-center gap-3 h-10 px-3 hover:bg-gray-50 rounded-sm text-left ${
...
export function CountrySelector({ onCountryChange, variant = 'compact' }: CountrySelectorProps) {
  ...
  useEffect(() => {
    ...
    const defaultCountryCode = savedCountryCode || detectUserCountry();
    
    // Find country by matching country calling code or name
